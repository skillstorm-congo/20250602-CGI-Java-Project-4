...
public class TimesheetService {
	...
	public Timesheet approve(int id, int managerId) {
		Map<String,Object> me = userServiceClient.userEmployee();
		int callerEmpId = getInt(me, "employeeId");
	    ...
	    if (callerEmpId != managerId) {
	        throw new ResponseStatusException(HttpStatus.FORBIDDEN, "Cannot act as another manager.");
	      }
	  
	    //pull employees under manager from the employee-service
	    var employeesResp = employeeServiceClient.findByManagerId(callerEmpId);
	    var employees = employeesResp.getBody();
	    boolean managesThisEmployee = false;
	    if (employees != null) {
	    	for (var e : employees) {
	    		if (e.getId() == t.getEmployeeId()) { managesThisEmployee = true; break; }
	            }
	    	}
	...
	@Transactional
	public Timesheet unapprove(int id, int managerId) {
		Map<String,Object> me = userServiceClient.userEmployee();
		int callerEmpId = getInt(me, "employeeId");
	...
	private static <T> void copyIfNotNull(T v, Consumer<T> set) {
